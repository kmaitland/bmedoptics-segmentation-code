...
red = filled_mask - green; % Image 1 non-overlap
blue = filled_mask2 - green;% Image 2 non-overlap
...
num_green = max(max(bwlabel(img1out)));
...
num_blue = max(max(bwlabel(img2out)));
...
num_red = max(max(bwlabel(green)));

% Display stuff
disp(['# of Overlapping objects (green) = ' num2str(num_red)]);
disp(['# of Non-overlapping image1 objects (red) = ' num2str(num_green)]);
...
disp(['Non-overlapping image1 objects (red) = ' num2str(sum(area1))]);
disp(['Non-overlapping image2 objects (blue)= ' num2str(sum(area2))]);